...
export class TvShowsListProviderService {
  ...
  private dataLoader$: Observable<TvShow[]> = toObservable(
    this.filterQuerySignal,
  ).pipe(switchMap((query: string | null) => this.loadData(query)));
  ...
  private loadData(query: string | null): Observable<TvShow[]> {
    const source$: Observable<TvShowsPagedCollectionResponse> =
      query && query.length ? this.api.search(query) : this.api.popularList();
